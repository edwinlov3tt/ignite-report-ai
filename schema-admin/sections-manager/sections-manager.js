...
class SectionsManager {
    ...
    bindEvents() {
        ...
        const testSectionBtn = document.getElementById('test-section-btn');
        if (testSectionBtn) {
            testSectionBtn.addEventListener('click', () => {
                // Navigate to AI Testing page instead of modal
                window.location.href = '../../ai-testing/';
            });
        }

        // Header test button
        const testSectionsBtn = document.getElementById('test-sections-btn');
        if (testSectionsBtn) {
            testSectionsBtn.addEventListener('click', () => {
                window.location.href = '../../ai-testing/';
            });
        }

        // Scope selector functionality
        const sectionScope = document.getElementById('section-scope');
        if (sectionScope) {
            sectionScope.addEventListener('change', (e) => {
        ...
        if (sectionProduct) {
            sectionProduct.addEventListener('change', (e) => {
    ...
    populateProductDropdown() {
        const select = document.getElementById('section-product');
    ...
    handleScopeChange(scope) {
        const productGroup = document.getElementById('product-selector-group');
        const subproductRow = document.getElementById('subproduct-selector-row');
        
        if (scope === 'product') {
            productGroup.style.display = 'block';
            subproductRow.style.display = 'none';
        } else if (scope === 'subproduct') {
            productGroup.style.display = 'block';
            subproductRow.style.display = 'block';
        } else {
    ...
    handleProductChange(productName) {
        const subproductSelect = document.getElementById('section-subproduct');
        subproductSelect.innerHTML = '<option value="">Choose a subproduct...</option>';
        
        if (productName) {
            const product = this.products.find(p => p.name === productName);
            if (product && product.subproducts) {
                const subproducts = Array.isArray(product.subproducts) ? 
                    product.subproducts : Object.values(product.subproducts);
                    
                subproducts.forEach(subproduct => {
                    const option = document.createElement('option');
                    option.value = subproduct.name;
